...
import com.knocklock.presentation.lockscreen.util.DismissDirection.*
import com.knocklock.presentation.lockscreen.util.DismissValue.*
import kotlinx.coroutines.CancellationException
import kotlin.math.roundToInt
/**
 * @Created by 김현국 2023/01/30
 * @Time 3:45 PM
 */
...
internal fun SwipeToDismiss(
    ...
    val minFactor =
        if (EndToStart in directions) SwipeableDefaults.StandardResistanceFactor else SwipeableDefaults.StiffResistanceFactor
    val maxFactor =
        if (StartToEnd in directions) SwipeableDefaults.StandardResistanceFactor else SwipeableDefaults.StiffResistanceFactor
    Box(
        Modifier.swipeable(
            state = state,
            anchors = anchors,
